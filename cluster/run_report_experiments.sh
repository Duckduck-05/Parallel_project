...
#           exp_speedup (speedup with & without comm).
#
# Single machine (this node, procs = cores):
#   bash cluster/run_report_experiments.sh
# On the cluster (distributed, via run_cluster.sh):
#   bash cluster/run_report_experiments.sh cluster/hosts
#
...
# --- locate OpenMPI + a python with numpy/matplotlib (the solver is C++; python only plots) ---
...
for c in /opt/homebrew/bin/python3 python3 /usr/bin/python3; do
  command -v "$c" >/dev/null 2>&1 && "$c" -c "import numpy,matplotlib" >/dev/null 2>&1 && PY="$c" && break
done
[ -n "$PY" ] || { echo "ERROR: need python3 with numpy + matplotlib"; exit 1; }
