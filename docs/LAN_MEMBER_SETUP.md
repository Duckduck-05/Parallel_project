...
## STEP 2 — (ALL WSL machines: node1/bao, node3/tin, node4/khai) open Windows inbound
Windows blocks LAN traffic into mirrored WSL — so other nodes can't reach you (and can't
connect back to the launcher). EVERY WSL machine must do this; node2/duc (native Linux)
skips it. Fix on the **Windows side** (NOT inside WSL):
