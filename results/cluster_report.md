...
giảm comm xuống gần 0. Giải thích: với 48 rank trải trên 4 máy không đồng nhất, phần lớn "comm
time" đo được thực chất là **thời gian chờ ở rào đồng bộ (barrier) để rank chậm nhất bắt kịp**,
không phải chi phí truyền dữ liệu của riêng broadcast. Giảm tần suất sync (sync lớn hơn) không
giảm tổng thời gian chờ vì lệch tiến độ giữa rank nhanh/chậm chỉ dồn lại thành các lần chờ lâu
hơn nhưng ít lần hơn — về tổng thể gần như không đổi. **Kết luận: với cluster KHÔNG đồng nhất,
nút thắt comm chủ yếu do mất cân bằng tốc độ giữa các máy, không phải do tần suất migration** —
khác với kết luận ở mục 4.3 (vốn áp dụng cho trường hợp N nhỏ, nơi chi phí round-trip latency
của chính các lệnh collective mới là yếu tố chính). Cả hai hiệu ứng cùng tồn tại; ở quy mô
N=4800/48-rank này, hiệu ứng mất cân bằng tốc độ máy lấn át hiệu ứng latency thuần.

**Lưu ý quan trọng về cột "Best length" ở bảng trên**: mỗi dòng chỉ là **1 lần chạy duy nhất**
(1 seed), KHÔNG chạy lại nhiều seed để lấy trung bình/phương sai. Vì vậy **không nên kết luận
rằng sync=0/20/100/200 cái nào cho chất lượng nghiệm tốt hơn cái nào** chỉ từ bảng này — sai
biệt giữa 174825 và 175584 (~0.4%) hoàn toàn có thể nằm trong nhiễu ngẫu nhiên giữa các lần
chạy GA (stochastic), không phải hiệu ứng thật của tần suất migration. Muốn kết luận chắc về
chất lượng nghiệm cần chạy lại mỗi giá trị sync với ≥5-10 seed khác nhau và so sánh trung bình
± độ lệch chuẩn - ngoài phạm vi thời gian của báo cáo này, nêu rõ như một hướng mở rộng.
...
Đáng chú ý: `lscpu` cho thấy node3 có **CPU cùng model với node4** (Intel i5-12500H, 8 core/16
thread) nhưng RAM thấp hơn nhiều (3.7GB so với 7.7GB). Tuy nhiên **chỉ với 1 lần đo mỗi cấu
hình (không lặp lại nhiều lần), không thể khẳng định tuyệt đối nguyên nhân chậm là "thuần do
RAM"** — đây là suy luận hợp lý nhất từ các dữ kiện đã có (cùng CPU, khác RAM, không tới mức
swap), nhưng cũng có thể do các yếu tố khác của hệ thống không kiểm soát được (process khác
đang chạy trên máy đó, nhiệt độ/throttling, biến thiên ngẫu nhiên của GA). Diễn đạt chính xác
hơn: **node3 chậm hơn nhiều khả năng do áp lực bộ nhớ kết hợp với sự không đồng nhất hệ thống
nói chung (memory pressure / general system heterogeneity), chưa đủ cơ sở để cô lập RAM là
nguyên nhân duy nhất** — cần đo lặp lại nhiều lần (và lý tưởng là theo dõi `vmstat`/cache-miss
counter trong lúc chạy) để kết luận chắc chắn hơn.

### 14.3 Kết luận tổng hợp

1. **node3 là straggler nhẹ** (chậm hơn ~8%, nhiều khả năng liên quan đến RAM thấp hơn so với
   node4 dù cùng CPU - xem lưu ý ở trên về giới hạn của kết luận này) - loại nó ra *đúng cách*
   (giữ cap HT) giúp nhanh hơn.
...
- N*/gens* search: số liệu trong mục 8
...
- **`results/nstar_run.log`** - stdout đầy đủ của lần chạy N*/gens* cuối (N=2400, gens=10500,
  procs=48, sync=20); **`results/nstar_final_stats.csv`** - stats CSV per-rank của lần chạy đó
- **`results/speedup_run.log`** - stdout đầy đủ của lần chạy speedup cuối (N=4800, gens=400,
  sync=20, procs=1..48, qua `experiments.py speedup`)
...
## 16. Contribution theo thành viên

**Hạn chế cần lưu ý**: tôi (Claude Code) chỉ có thể đọc được dữ liệu khách quan từ git, KHÔNG
biết thực tế ai đã làm việc gì (vai trò, mức độ đóng góp chất lượng) - phần này cần chính nhóm
tự điền dựa trên thực tế làm việc, tôi chỉ cung cấp số liệu thô làm điểm khởi đầu.

**Dữ liệu thô từ `git shortlog -sn --all` + `git log --numstat`:**

| Tác giả (theo git) | Số commit | Dòng thêm | Dòng xoá |
|---|---|---|---|
| Quốc Bảo | 29 | +17833 | -7843 |
| Duck | 8 | +27599 | -121 |

(Lưu ý: số dòng thêm/xoá ở đây tính TẤT CẢ file trong git history, gồm cả `data/`, `results/`,
`report/` (có thể chứa file lớn như `.docx`) - **không phải con số LOC code sạch** đã dùng ở
mục 13. Số commit nhiều hơn không tự động nghĩa là đóng góp nhiều hơn về chất lượng/độ khó.)

**Theo username máy chủ trong cluster** (suy ra từ SSH config, KHÔNG chắc phản ánh đúng người
viết code - có thể chỉ là người cung cấp máy):

| Username | Máy | Vai trò hạ tầng (suy đoán từ việc setup) |
|---|---|---|
| duck | node2 (launcher) | Vận hành cluster, chạy thí nghiệm (phần việc trong session này) |
| bao | node1 | Cung cấp máy node1 |
| acer | node3 | Cung cấp máy node3 |
| khainx | node4 | Cung cấp máy node4 |

**Đề xuất**: nhóm nên tự bổ sung bảng phân công thực tế (ví dụ: ai viết `ga_core.hpp`, ai viết
`tsp_island.cpp`/MPI, ai viết phần Python visualization, ai phụ trách hạ tầng cluster/SSH) -
tôi không có cơ sở để khẳng định điều này chỉ từ git log hoặc tên máy.
